...
export = class Text extends BaseVisualComponent {
  ...
  calculateSize() {
    ...
    const height =
      (characters[0][0].length + this.fontFamily.definition.spacing.top + this.fontFamily.definition.spacing.bottom) *
      characters[0].length
    let width = 0
  ...
  generateBinaryImage(
    ...
    for (let line of characters) {
      ...
      for (let character of line) {
        ...
        textOffsetLeft +=
          character[0].length + this.fontFamily.definition.spacing.left + this.fontFamily.definition.spacing.right
        for (var y = 0; y < character.length; y++) {
          for (var x = 0; x < character[0].length; x++) {
            ...
            if ((yIndex > 0 && yIndex < binaryBase.length && xIndex > 0 && xIndex < binaryBase[yIndex].length) == false)
              continue
            if (value) {
              if (this.invert) {
